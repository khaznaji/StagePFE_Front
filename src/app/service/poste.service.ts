...
export class PosteService {
  ...
  editPoste(postId: number, titre?: string, description?: string, nombrePostesDisponibles?: number, competences?: any[]): Observable<any> {
    const url = `${this.BASE_URL2}/edit/${postId}?titre=${titre}&description=${description}&nombrePostesDisponibles=${nombrePostesDisponibles}&competences=${competences}?`;
    const authToken = this.authService.getToken();
    const headers = new HttpHeaders({
        'Authorization': `Bearer ${authToken}`,

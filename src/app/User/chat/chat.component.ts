...
export class ChatComponent implements OnInit, AfterViewChecked {
  ...
  ngOnInit(): void {
    this.userService.getUserById(this.route.snapshot.paramMap.get('user')!).subscribe((data: any) => {
      this.otherUser = data;
      this.connectToChat();
      console.log(this.el);
  ...
  image!:string;
  getUserByid(id: any) {
    const headers = { 'Authorization': 'Bearer ' + this.userAuthService.getToken() };
...
this.connectToChat();
  ...
  connectToChat() {
    ...
    const id1 = this.userAuthService.getId()
      .toString()
      .replace('"', '')
      .replace('"', '');
    console.log('id1:', id1);
    
    const nick1 = this.username
      .replace('"', '')
      .replace('"', '');
    ...
    this.stompClient.connect(
        ...
        (frame) => {
          ...
          console.log('Subscribing to topic: ' + '/topic/messages/' + this.channelName);
  ...
  sendMsg() {
    if (this.newMessage.value !== '' && this.stompClient) { // Vérifiez si this.stompClient est défini
      const authToken = this.userAuthService.getToken();
  ...
  loadChat() {
    ...
    this.messages = this.http.post<Array<Messaggio>>(this.url+'/getMessages' ,  this.channelName , {headers});

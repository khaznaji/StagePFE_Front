...
export class UpdateBilanComponent implements OnInit {
    ...
    this.bilanAnnuelService.getBilanById(bilanAnnuelId).subscribe(
      (data) => {
        ...
        this.bilanAnnuelMiseAJour = {
          objectifAtteints: this.bilanAnnuel.objectifAtteints,
          objectifsFuturs: this.bilanAnnuel.objectifsFuturs,
          projetsAccomplis: this.bilanAnnuel.projetsAccomplis,
          challenges: this.bilanAnnuel.challenges
        };
      },
      (error) => {
  ...
  mettreAJourBilanAnnuel() {
    this.bilanAnnuelService.mettreAJourBilanAnnuel(this.bilanAnnuelId, this.bilanAnnuelMiseAJour).subscribe(
      (data) => {
      ...
      (error) => {
        console.error('Une erreur est survenue lors de la mise à jour du bilan annuel : ', error);
      }
    );
  }
  mettreAJouretEnvoyeBilanAnnuel() {
    this.bilanAnnuelService.mettreAJouretEnvoyeBilanAnnuel(this.bilanAnnuelId, this.bilanAnnuelMiseAJour).subscribe(
      (data) => {
        console.log('Bilan annuel mis à jour avec succès : ', data);
        // Redirigez l'utilisateur vers une autre page après la mise à jour, par exemple la liste des bilans
      },

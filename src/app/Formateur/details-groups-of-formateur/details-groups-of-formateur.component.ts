...
export class DetailsGroupsOfFormateurComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.participationFormation.getFormationsConfirme(this.id).subscribe(
      ...
      (error) => {
        console.error(
          'Erreur lors de la récupération des collaborateurs confirmés : ',
          error
        );
      }
    );
  ...
  editGroup() {
    this.groupsService.editGroupName(this.groupId, this.nom).subscribe(
      (response) => {
      ...
      (error) => {
        console.error(
          'Une erreur est survenue lors de la modification du nom du groupe : ',
          error
        );
  ...
  removeUserFromGroup(userId: number): void {
    if (
      confirm('Êtes-vous sûr de vouloir supprimer cet utilisateur du groupe?')
    ) {
      this.groupsService.removeUserFromGroup(this.groupId, userId).subscribe(
        (response) => {
  ...
  getGroupesDetails(): void {
    this.groupsService.getGroupesDetails(this.groupId).subscribe(
      (data: any[]) => {
        this.groupes = data;
      },
      (error) => {
  ...
  ajouterGroupe(): void {
    const collaborateursId = (
      this.groupForm?.get('collaborateursId') as FormArray
    ).controls
      .map((control, index) =>
        control.value ? this.collaborateursConfirme[index].idC : null
      )
      .filter((id) => id !== null); // Filtrer les valeurs null

    if (collaborateursId.length > 0) {
      this.groupsService
        .addCollaborateursToGroup(this.id, this.groupId, collaborateursId)
        .subscribe(
          (response) => {
  ...
  saveCertif() {
    ...
    this.certificateService.create(this.groupId, formData).subscribe(
      (data) => {
        ...
        Swal.fire({
          title: 'Success',
          text: 'Les Certificats ont été générées avec succès.',
          icon: 'success',
  ...
  deleteCertificates(groupId: number): void {
    Swal.fire({
      title: 'Etes vous sur?',
      text: 'Vous êtes sur le point de supprimer les certificats de ce groupe. Cette action ne peut pas être annulée.',
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        ...
        this.certificateService.deleteCertificatesForGroup(groupId).subscribe(
          () => {
            ...
            Swal.fire(
              'Supprimé!',
              'Les Certificats ont été supprimée avec succés',
              'success'
            );
          },
          (error) => {
            ...
            Swal.fire('Error!', 'Failed to delete certificates.', 'error');
          }
        );
      }
    });
    ...
    if (this.certificateForm.valid) {
      ...
      this.certificateService.update(this.groupId, formData).subscribe(
        (response) => {
          ...
          Swal.fire({
            title: 'Success',
            text: 'Les Certificates on été modifiée avec succés .',
            icon: 'success',
            confirmButtonText: 'OK',
          }).then(() => {
